<?php
$pageTitle = "Christer Barregren | Poems";
$activeNav = "poems";
$heroImage = null;
include __DIR__ . "/../partials/header.php";

$postsDir = __DIR__ . "/poems-data";
$perPage  = 12;
...
function list_php_posts(string $postsDir): array {
  ...
  foreach ($files as $file) {
    ...
    $footer_html = isset($data['footer_html']) ? (string)$data['footer_html'] : '';
...
<section class="section" aria-label="Dikter">
  <div class="paper">
      ...
      <?php if (!empty($single['tags'])): ?>
        <div class="poem-tags">
          ...
          <?php foreach ($single['tags'] as $tag): ?>
            <a class="blog-tag" href="/en/poems/?tag=<?php echo urlencode($tag); ?>"><?php echo h($tag); ?></a>
          <?php endforeach; ?>
        </div>
      <?php endif; ?>
      <a class="card__link" href="/en/poems/">Back</a>
    </div>
...
<?php else: ?>
        ...
        <header class="intro intro--poem">
          <h1>Poems</h1>
          <?php if ($activeTag !== ''): ?>
            <p class="subtitle">Labels: <strong><?php echo h($activeTag); ?></strong> &nbsp;·&nbsp; <a class="textlink" href="/en/poems/">Show all</a></p>
          <?php else: ?>
            <p class="subtitle">From a world where no one ever does what is required</p>
          <?php endif; ?>
        </header>
          ...
          <div class="reading">
              ...
              <article class="card saga-item">
                <div class="card__body">
                  <h2 class="card__title">
                    <a href="<?php echo url('/poems/?slug=' . h($p['slug'])); ?>">
                      <?php echo h($p['title']); ?>
                    </a>
                  </h2>
                    ...
                    <p class="blog-tags">
                      <?php foreach ($p['tags'] as $tag): ?>
                        <a class="blog-tag <?php echo $tag === $activeTag ? 'blog-tag--active' : ''; ?>"
                           href="/en/poems/?tag=<?php echo urlencode($tag); ?>"><?php echo h($tag); ?></a>
                      <?php endforeach; ?>
                    </p>
                  <?php endif; ?>
                  ...
                  <a class="card__link" href="<?php echo url('/poems/?slug=' . h($p['slug'])); ?>">Read</a>
                </div>
              </article>
            <?php endforeach; ?>
            ...
            <?php if ($totalPages > 1): ?>
              <nav style="display:flex; align-items:center; justify-content:space-between; gap:14px; margin-top:24px;">
                <div>
                  <?php if ($page > 1): ?>
                    <a class="card__link" href="<?php echo url('/poems/?page=' . ($page - 1) . ($activeTag !== '' ? '&tag=' . urlencode($activeTag) : '')); ?>">Previous</a>
                  <?php endif; ?>
                </div>
                ...
                <div>
                  <?php if ($page < $totalPages): ?>
                    <a class="card__link" href="<?php echo url('/poems/?page=' . ($page + 1) . ($activeTag !== '' ? '&tag=' . urlencode($activeTag) : '')); ?>">Next</a>
                  <?php endif; ?>
                </div>
              </nav>
            <?php endif; ?>
