<?php
$pageTitle = "Christer Barregren | Dikter";
$activeNav = "dikter";
$heroImage = null;
include __DIR__ . "/../partials/header.php";

$postsDir = __DIR__ . "/dikter-data";
$perPage  = 12;
      ...
      <?php if (!empty($single['tags'])): ?>
        <div class="poem-tags">
          ...
          <?php foreach ($single['tags'] as $tag): ?>
            <a class="blog-tag" href="/dikter/?tag=<?php echo urlencode($tag); ?>"><?php echo h($tag); ?></a>
          <?php endforeach; ?>
        </div>
      <?php endif; ?>
      <a class="card__link" href="/dikter/">Tillbaka</a>
    </div>
...
<?php else: ?>
        ...
        <header class="intro intro--poem">
          <h1>Dikter</h1>
          <?php if ($activeTag !== ''): ?>
            <p class="subtitle">Etikett: <strong><?php echo h($activeTag); ?></strong> &nbsp;·&nbsp; <a class="textlink" href="/dikter/">Visa alla</a></p>
          <?php else: ?>
            <p class="subtitle">från en värld där ingen någonsin gör vad som krävs</p>
          <?php endif; ?>
        </header>

        <?php if ($totalPosts === 0): ?>
          <div class="reading prose">
            <p>Inga dikter ännu.</p>
          </div>
        <?php else: ?>
          <div class="reading">
              ...
              <article class="card saga-item">
                <div class="card__body">
                  <h2 class="card__title">
                    <a href="<?php echo url('/dikter/?slug=' . h($p['slug'])); ?>">
                      <?php echo h($p['title']); ?>
                    </a>
                  </h2>
                    ...
                    <p class="blog-tags">
                      <?php foreach ($p['tags'] as $tag): ?>
                        <a class="blog-tag <?php echo $tag === $activeTag ? 'blog-tag--active' : ''; ?>"
                           href="/dikter/?tag=<?php echo urlencode($tag); ?>"><?php echo h($tag); ?></a>
                      <?php endforeach; ?>
                    </p>
                  <?php endif; ?>
                  ...
                  <a class="card__link" href="<?php echo url('/dikter/?slug=' . h($p['slug'])); ?>">Läs</a>
                </div>
              </article>
            <?php endforeach; ?>
