<?php
$pageTitle = "Christer Barregren | Sagor";
$activeNav = "sagor";
$heroImage = null;
include __DIR__ . "/../partials/header.php";

$postsDir = __DIR__ . "/sagor-data";
$perPage  = 12;
    ...
    <div class="paper__inner">
          ...
          <?php if (!empty($single['tags'])): ?>
            <div class="poem-tags">
              ...
              <?php foreach ($single['tags'] as $tag): ?>
                <a class="blog-tag" href="/sagor/?tag=<?php echo urlencode($tag); ?>"><?php echo h($tag); ?></a>
              <?php endforeach; ?>
            </div>
          <?php endif; ?>

          <p class="backline">
            <a class="card__link" href="/sagor/">Tillbaka</a>
          </p>
        </article>
      ...
      <?php else: ?>
        <header class="intro">
          <h1>Sagor</h1>
          <?php if ($activeTag !== ''): ?>
            <p class="subtitle">Etikett: <strong><?php echo h($activeTag); ?></strong> &nbsp;·&nbsp; <a class="textlink" href="/sagor/">Visa alla</a></p>
          <?php else: ?>
            <p class="subtitle">Allt du berättar är en saga</p>
          <?php endif; ?>
        </header>
              ...
              <article class="card saga-item">
                <div class="card__body">
                  <h2 class="card__title">
                    <a href="/sagor/?slug=<?php echo h($p['slug']); ?>">
                      <?php echo h($p['title']); ?>
                    </a>
                  </h2>
                    ...
                    <p class="blog-tags">
                      <?php foreach ($p['tags'] as $tag): ?>
                        <a class="blog-tag <?php echo $tag === $activeTag ? 'blog-tag--active' : ''; ?>"
                           href="/sagor/?tag=<?php echo urlencode($tag); ?>"><?php echo h($tag); ?></a>
                      <?php endforeach; ?>
                    </p>
                  <?php endif; ?>
                  ...
                  <a class="card__link" href="/sagor/?slug=<?php echo h($p['slug']); ?>">Läs</a>
                </div>
              </article>
            <?php endforeach; ?>
